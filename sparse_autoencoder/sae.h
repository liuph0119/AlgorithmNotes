...
public:
	...
	bool trainSparseAutoEncoder2(Matrix X, int nHiddenSize = 25, double lr = 1.0, int nTrainingIter = 400, int nBatchSize = 0, double lambda = 0.0001, double sparseParam = 0.1, double beta = 3)
	{
		mMatData = X;
		mnSampleNum = X.ColNo();
		...
		// 确定 batch size
		if (nBatchSize == 0) nBatchSize = mMatData.ColNo() / 100;
		...
		// 设置优化器;
		Optimizer *optimizer1 = new_Optimizer("Adam", mParams.lr);
		Optimizer *optimizer2 = new_Optimizer("Adam", mParams.lr);
		cout << "optimizing by " << static_cast<Adam*>(optimizer1)->name().c_str() << "..."<< endl;
		...
		for (int i_iter = 0; i_iter < mParams.nMaxIter; i_iter++)
		{
			...
			// 计算损失函数值和梯度;
			cout << "\r" <<" # iter " << setw(4) << i_iter + 1 << " # \tCOST: ";
			double dCost = 0;
			std::vector<Matrix> vGrad = calCost(data, dCost);
			cout << " = " << setw(6) << setprecision(3) << dCost << "\t";

			// 优化权重;
			optimizer1->update_weights(mTheta[0], mTheta[2], vGrad[0], vGrad[2]);
			optimizer2->update_weights(mTheta[1], mTheta[3], vGrad[1], vGrad[3]);
			vGrad.clear();
		}
		cout << endl;
		return true;
...
private:
	...
	std::vector<Matrix> calCost(Matrix &data, double &dCost)
	{
		...
		// 在原始数据中加入噪声;
		Matrix noise_data = multiplyMat(data, getBernoulliMatrix(data.RowNo(), data.ColNo(), 0.05));
		Matrix active_value2 = sigmoid(W1*data + repmat(B1, 1, nSample));
