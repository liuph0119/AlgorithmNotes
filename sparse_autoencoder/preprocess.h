...
//#pragma omp parallel for schedule(dynamic)
	...
	{
		for (int j = 0; j < mat.ColNo(); j++)
			_mat(i, j) = std::pow(mat(i, j), _pow);
	}
	return _mat;
}

Matrix sqrtMat(Matrix mat)
{
	Matrix _mat(mat.RowNo(), mat.ColNo());
//#pragma omp parallel for schedule(dynamic)
	for (int i = 0; i < mat.RowNo(); i++)
...
Matrix repmat(Matrix mat, int r, int c)
{
	...
	if (mat.RowNo() == 1 && r != 1)	// 1行c列，将这1行复制到r行
	{
	...
	else if (mat.ColNo() == 1 && c != 1)	// 1列r行,将这1列复制到c列
	{
...
#pragma omp parallel for schedule(dynamic)
	for (int i = 0; i < mat.ColNo(); i++)
	{
		// 矩阵值减去该列的均值;
		for (int j = 0; j < mat.RowNo(); j++)
			mat(j, i) -= mat_mean(0, i);
	}

	// 全部元素的均值;
	double dMeanAll = meanMat(mat);
...
bool normalizeData(Matrix &mat, int METHOD)
{
	Matrix _miu = meanMat(mat, 0);
	mat = mat - repmat(_miu, mat.RowNo(), mat.ColNo());	// X = X - μ

	Matrix sigma2 = sumMat(multiplyMat(mat, mat), 0) / (double)mat.ColNo();	//σ^2

	mat = devisionMat(mat, sigma2);			// X = X/σ2
...
double cosin(Matrix v1, Matrix v2)
{
	...
	double dv1 = sqrt(sumMat(powMat(v1, 2.0)));
	double dv2 = sqrt(sumMat(powMat(v2, 2.0)));
